...
Также есть два режима освещения: программный, когда нету нейросети для переосвещения, и соответственно нейросетевой, когда такая модель загружена.

Для генерации карты цвета также есть два режима: если выход модели (x, 64, 64, 3) — значит это RGB модель, если выход (x, 64, 64, 2) — значит модель использует HSL

В самой правой части обзорщик глубины, создающий гифку на основе карт глубины, а также генератор финальной гифки из карт света, также можно сохранить все отдельные карты картинкой. В поле выводится адрес полученного изображения или гифки.

![image](https://github.com/user-attachments/assets/25cf763c-bfe4-4e0d-9815-2b039c257668)
